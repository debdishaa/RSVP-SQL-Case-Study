...
-- id, title, year,date_publihsed, duration has null values
...
SELECT COUNT(*) AS count_of_single_genre_movie
...
+------------------+-------------------+---------------------+*/
-- Type your code below:
-- Using CTE to create temporary storage and extracting value according to rank
WITH hit_production_company
...
CASE
    ...
    END AS 'IsGermanMoviePopularThan Italian?'
FROM german_movie
     INNER JOIN italian_movie
USING(unique_id); 
...
WITH top_three_ranked_genres AS (
SELECT genre,
	   COUNT(m.id) AS movie_count
    FROM movie m
    INNER JOIN genre g 
...
(
    ...
    INNER JOIN genre AS g 
    ON m.id= g.movie_id
	WHERE genre IN (SELECT genre FROM top_three_ranked_genres)
)
SELECT *
FROM top_five_ranked_movie
WHERE movie_rank<=5;
...
WITH publish_duedate_summary AS
(
           SELECT     d.name_id,
                      Name,
                      d.movie_id,
                      ...
                      total_votes,
                      m.date_published,
                      LEAD(date_published,1) OVER(partition BY d.name_id ORDER BY date_published,movie_id ) AS publish_duedate
           FROM       director_mapping  AS d
           INNER JOIN names AS n
           ON         n.id = d.name_id
           INNER JOIN movie AS m
...
(
SELECT *,
	   Datediff(publish_duedate, date_published) AS date_difference
FROM   publish_duedate_summary )
