...
--null values are present in country, world_gross_income, languages and production_company
...
SELECT COUNT(id) AS number_of_movies, EXTRACT(YEAR FROM date_published) AS yearr
FROM movie
WHERE country IN ('USA', 'India')
GROUP BY country, EXTRACT(YEAR FROM date_published)
HAVING EXTRACT(YEAR FROM date_published) = 2019;
...
-- Drama has a total number of 4285  movies
...
-- 3289 mvies belong to only one genre
...
SELECT name, total_votes, movie_count, avg_rating, actress_rank
FROM (
    SELECT n.name,
           SUM(r.total_votes) AS total_votes,
...
WITH movie_date_information AS (
    SELECT d.name_id, n.name, d.movie_id,
           m.date_published, 
           LEAD(date_published) OVER(PARTITION BY d.name_id ORDER BY date_published, d.movie_id) AS next_movie_date
...
date_diff AS (
    SELECT name_id, 
           AVG(next_movie_date - date_published) AS avg_inter_movie_days
...
final_output AS (
    ...
    JOIN date_diff AS dd ON dd.name_id = d.name_id
    GROUP BY director_id, director_name, avg_inter_movie_days  -- Group by avg_inter_movie_days
)
SELECT *	
FROM final_output
LIMIT 9;
